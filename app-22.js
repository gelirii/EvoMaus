...
(() => {
  ...
  // Remove only after legacy scripts have initialised, so their startup listeners stay safe.
  ...
  function currentVisible(mouse) {
    ...
    for (let y = Math.max(0, mouse.y - FOV_RADIUS); y <= Math.min(GRID_H - 1, mouse.y + FOV_RADIUS); y++) {
      for (let x = Math.max(0, mouse.x - FOV_RADIUS); x <= Math.min(GRID_W - 1, mouse.x + FOV_RADIUS); x++) {
        const dx = x - mouse.x, dy = y - mouse.y;
        if (dx * dx + dy * dy <= r2 && lineVisible(mouse.x, mouse.y, x, y)) visible.add(numericKey(x, y));
      }
    }
    fovCacheKey = key;
    fovCache = visible;
  ...
  function drawTrueFog(mouse) {
    // Edit mode is deliberately fully visible; Resume restores the same checked Mouse View.
    ...
    const a = screenToWorld(0, 0), b = screenToWorld(rect.width, rect.height);
    const minX = Math.max(0, Math.floor(a.x) - 1), maxX = Math.min(GRID_W - 1, Math.ceil(b.x) + 1);
    const minY = Math.max(0, Math.floor(a.y) - 1), maxY = Math.min(GRID_H - 1, Math.ceil(b.y) + 1);
  ...
  function drawDistinctMouse(mouse) {
    ...
    const s = Math.max(6, scale * .58), x = p.x + scale / 2 - s / 2, y = p.y + scale / 2 - s / 2;
    ctx.save();
    ctx.globalAlpha = 1;
    ctx.fillStyle = style.body; ctx.strokeStyle = style.outline; ctx.lineWidth = Math.max(1.2, scale * .045);
    roundRect(ctx, x, y, s, s, Math.max(2, s * .16)); ctx.fill(); ctx.stroke();
    if (s > 8) {
      ctx.fillStyle = style.ears; ctx.beginPath();
      ctx.arc(x + s * .21, y + s * .12, s * .16, 0, Math.PI * 2);
      ctx.arc(x + s * .79, y + s * .12, s * .16, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = style.outline; ctx.lineWidth = Math.max(.8, scale * .025); ctx.stroke();
      ctx.fillStyle = style.eye; ctx.beginPath();
      ctx.arc(x + s * .31, y + s * .43, Math.max(1, s * .06), 0, Math.PI * 2);
      ctx.arc(x + s * .69, y + s * .43, Math.max(1, s * .06), 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = style.nose; ctx.beginPath();
      ctx.arc(x + s * .5, y + s * .69, Math.max(1, s * .075), 0, Math.PI * 2); ctx.fill();
    }
    ctx.restore();
  }
  ...
  function drawAllMiceDistinctly() {
    ...
    for (const mouse of state.mice) if (mouse.alive || mouse.reachedGoal) drawDistinctMouse(mouse);
  }
  ...
  function drawTargetGlow(mouse) {
    ...
    glow.addColorStop(0, 'rgba(45,174,255,.22)'); glow.addColorStop(.62, 'rgba(45,174,255,.13)'); glow.addColorStop(1, 'rgba(45,174,255,0)');
    ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(p.x, p.y, radius, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = 'rgba(42,174,255,.95)'; ctx.lineWidth = Math.max(2, state.view.scale * .055);
    ctx.beginPath(); ctx.arc(p.x, p.y, radius * .72, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
  }
  ...
  function drawChampionTrail(lineage, trail) {
    ...
    ctx.strokeStyle = style.under; ctx.lineWidth = Math.max(4, state.view.scale * .16); ctx.stroke();
    ctx.strokeStyle = style.main; ctx.lineWidth = Math.max(1.8, state.view.scale * .075); ctx.stroke(); ctx.restore();
  }

  function drawStrongChampionTrails() {
    if (state.mode !== 'sim' || document.getElementById('v13TrailsToggle')?.checked === false) return;
  ...
  // Suppress the legacy square/fade/debug-view while the old render chain runs, then
  // redraw all mice at full opacity, apply our fog, and add one constant circular glow.
  ...
  // ---- Richer generation summaries ------------------------------------------------
  function speciesSummary(lineage) {
    ...
    return {
      lineage,
      bestProgress,
      bestSteps: contenders.length ? Math.min(...contenders.map(m => m.step || 0)) : 0,
      cheese: Math.max(...group.map(m => m.cheeseCount || 0)),
      danger: group.filter(m => m.deathReason === 'danger').length,
      goals: group.filter(m => m.reachedGoal).length,
      total: group.length
    };
  }
  ...
  function richerSummaryHtml() {
    return `<div class="v134SummaryRows">${LINEAGES.map(lineage => {
      const s = speciesSummary(lineage); if (!s) return '';
      ...
      return `<div class="v134SpeciesSummary lineage-${lineage}">
        ...
        ${solved ? `<div class="v134GoalResult"><b>${s.goals}</b> reached goal · <b>${s.total - s.goals}</b> died</div>` : ''}
      </div>`;
    }).join('')}</div>`;
  }
  ...
  function compactPauseCards() {
    ...
    for (const card of pauseLineages.querySelectorAll('.v131PauseCard')) {
      ...
      if (alive) {
        const next = alive.textContent.replace(/\s*alive\s*$/i, '');
        if (next !== alive.textContent) alive.textContent = next;
      }
      if (record) {
        const next = record.textContent.replace(/^Record\s*·\s*/i, '');
        if (next !== record.textContent) record.textContent = next;
      }
    }
  }
